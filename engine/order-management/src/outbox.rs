...
impl DispatcherConfig {
    ...
    pub fn from_env() -> Option<Self> {
        ...
        Some(Self { url: url.trim().to_string(), secret: secret.trim().to_string(), sweep_interval: Duration::from_secs(sweep_secs.max(1)) })
    }
}
    ...
    for id in ids {
        match deliver(client, cfg, &id).await {
            ...
            Delivery::Failed(error) => {
                ...
                stats.failed += 1;
                if record_failure(pool, &id, &error).await? == FailureOutcome::Dead {
                    stats.dead += 1;
                }
            }
        }
