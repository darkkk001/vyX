...
type Scenario = {
  ...
  symbols: { name: string; contractSize: Dec; digits: number; quoteCurrency: string; category?: string; sessionClosedNow?: boolean; hedgedMarginPct?: Dec; spreadMarkup?: Dec }[];
...
async function main() {
  ...
  for (const file of files) {
    ...
    for (const s of sc.symbols) {
      ...
      const bs = await prisma.brokerSymbol.create({ data: { brokerId: broker.id, symbolId: row.id, ...(s.hedgedMarginPct != null ? { hedgedMarginPct: D(s.hedgedMarginPct) } : {}), ...(s.spreadMarkup != null ? { spreadMarkup: D(s.spreadMarkup) } : {}) } });
