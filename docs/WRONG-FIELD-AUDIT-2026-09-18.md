...
**Historical repair (fix/deep-backfill-full-history):** 0.1/0.2 left every tick-built bar since 2026-08-12 point-sampled and 0.3 left phantom `hh:mm:01` rows in the store. EA v1.40's `DeepBackfillFullHistory` pass re-sends the broker's own bars for every timeframe back to `DeepBackfillFromDate`; `retention::sweep_offgrid_candles` deletes the phantoms at engine boot; `scripts/candle-integrity-report.mjs` measures before/after. Steps: `deploy/deep-backfill-runbook.md`.
