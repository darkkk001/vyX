...
**Pool-sizing fix applied and verified (2026-08-17).** `db::connect_pool` now takes an explicit `max_connections` (via `PgPoolOptions`, not the bare `PgPool::connect` default of 10); `engine/server` reads it from a new `DATABASE_POOL_MAX_CONNECTIONS` env var, default 20. Re-ran the exact concurrency-20/200-request scenario that failed before: `PoolTimedOut` count dropped from 193/200 to 94/200 — a real, measured improvement, not a full fix. The remaining failures aren't a sizing bug anymore, they're this sandbox's DB latency itself: at concurrency 20, each in-flight request holds a connection for 15–30+ seconds (round-trip latency × ~8-10 sequential queries), so even 20 pool slots saturate before earlier requests finish. Two independent levers exist if concurrency 20 needs to fully succeed in a similarly high-latency environment: raise `DATABASE_POOL_MAX_CONNECTIONS` further (a config change, no code change), or reduce the number of sequential round-trips `place_market_order` makes per order (a real code change, not attempted here — batching the pre-transaction reads or folding them into the existing transaction is the likely shape of that fix, left as a follow-up, not silently bundled into this pass). Against a normal-latency (e.g. co-located) Postgres, the default of 20 is expected to be more than sufficient at this concurrency without either lever. Re-run this benchmark once either lever is pulled, or against a lower-latency Postgres, before treating any of these specific numbers as the real regression baseline; the *tooling and methodology* are the durable part of this pass. |
