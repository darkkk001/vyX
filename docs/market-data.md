...
| **S4** web, all | candles: `MARKET_DATA_VPS_SYMBOLS=*`. Prices: every LivePrice read goes through `lib/live-price.ts` (`getLivePriceRow`, `getLivePriceRows[WithSource]`, `getFreshPrices`) — with `MARKET_DATA_PRICES=vps` it reads the engine's tick cache (`/internal/prices[/{symbol}]`), Neon on any miss; callers: `/api/trade/prices` (header `x-market-data-source`), the three order routes, `lib/risk.ts`, `lib/mirror.ts`, `lib/pricing-shadow-compare.ts`, and via `getFreshPrices` the risk monitor, margin, bulk close, dealing queue, manage positions. Gateway `db.ts` reads LivePrice from `MARKET_DATA_DATABASE_URL` when set. | **code done** — dormant until `MARKET_DATA_PRICES=vps` (Vercel) / `MARKET_DATA_DATABASE_URL` (gateway) |
