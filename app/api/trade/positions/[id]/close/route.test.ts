...
async function createFixture(): Promise<Fixture> {
  ...
  const account = await prisma.account.create({
    data: { brokerId: broker.id, accountNumber: `7${suffix.slice(0, 7)}`, email: `cp-client-${suffix}@test.local`, passwordHash: "x", fullName: "Close Price Client", accountMode: "LIVE", balance: D(10000) },
  });
