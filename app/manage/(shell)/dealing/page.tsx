...
import DealingQueueManager, { type DealingOrderRow } from "./DealingQueueManager";
...
export default async function ManagerDealingPage() {
  ...
  const priceBySymbol = await getFreshPrices([...new Set(orders.map((o) => o.symbol.name))]);

  const rows: DealingOrderRow[] = orders.map((o) => {
  ...
  return (
    <main className="mx-auto max-w-[1400px]">
      <PageHeader
        title="Dealing queue"
        description={`${rows.length} order${rows.length === 1 ? "" : "s"} awaiting manual review. Only populated while dealing mode is on (Risk page).`}
      />
      <DealingQueueManager initialRows={rows} />
    </main>
  );
}
