...
export default function WebTrader({
  ...
  useEffect(() => {
    (async () => {
      try {
        await Promise.all([refreshAccount(), refreshPositions(), refreshOrders(), refreshAlerts()]);
      } catch (err) {
  ...
  function openPriceAlert(symbolName: string) {
    const mm = market[symbolName];
    askPrompt(`Alert me when ${symbolName} reaches:`, fmt(mm.bid, mm.def.digits), async (priceStr) => {
  ...
  const chartLines: ChartLine[] = useMemo(
    () => computeChartLines(activeSymbol, positions, pendingOrders, alerts),
    [positions, pendingOrders, activeSymbol, alerts]
  );
