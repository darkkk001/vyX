...
// public/admin-tauri-updates/, a separate feed from desktop-tauri's and
// manager-tauri's own, matching this app having its own signing keypair
// and pubkey in tauri.conf.json.
//
...
// signing private key (admin-tauri/src-tauri/.updater-keys/, gitignored,
...
const destDir = path.join(__dirname, "..", "public", "admin-tauri-updates");
const publicBaseUrl = "https://vyxtrader.com/admin-tauri-updates";
...
if (fs.existsSync(destDir)) {
  for (const existing of fs.readdirSync(destDir)) {
    if (existing.endsWith("-setup.exe") && existing !== installer) {
      ...
      console.log(`Removed stale installer: public/admin-tauri-updates/${existing}`);
    }
  }
}
...
console.log(`Copied ${installer} -> public/admin-tauri-updates/`);

const notesArgIndex = process.argv.indexOf("--notes");
const notes = notesArgIndex !== -1 ? process.argv[notesArgIndex + 1] : `VyXTrader Admin ${version}`;
...
console.log("Wrote public/admin-tauri-updates/latest.json");

console.log("\nNow commit and push the repo (public/admin-tauri-updates/) to publish this update to installed apps.");
