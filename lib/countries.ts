 ...
 *
 * Account.country was stored raw -- whatever the caller sent, trimmed. The
 * backoffice add-account form offered a free-text box hinted "ISO code,
 * optional", so "Pakistan", "XX" and "12345" all persisted into a field that
 * KYC and compliance read. This is the one list both sides validate against;
 * the backoffice ships its own copy for the picker (Vyx.Shared/Countries.cs),
 * generated from this same data, because the desktop app cannot import TS.
 *
 * Names are the common short form, not the UN long form -- a dealer typing
 * "Russia" should find it, not have to know "Russian Federation".
 */
export const COUNTRIES: ReadonlyArray<{ code: string; name: string }> = [
  ...
  { code: "IO", name: "British Indian Ocean Territory" }, { code: "BN", name: "Brunei" }, { code: "BG", name: "Bulgaria" },
  ...
  { code: "CG", name: "Congo" }, { code: "CD", name: "Congo (DRC)" }, { code: "CK", name: "Cook Islands" },
...
/** True for a valid ISO 3166-1 alpha-2 code. Caller upper-cases first. */
export function isCountryCode(code: string): boolean {
  return CODES.has(code);
}

/** Display name for a stored code, or the code itself if it is not one we know. */
